...
best.rubxii<-function(serie, sf, h=6, pmax=6, qmax=6, nbest=10,
                     tau=0.5,link = "logit",X=NA,X_hat=NA)
{
  ...
  fit<-rubxiiarma.fit(y, ma=1,diag=0,link = link, tau=tau,
                      X = X, X_hat=X_hat)
  aicmin<-fit$aic
  ...
  { 
    ...
    { 
      ...
      {   
        print(c(p,q),quote=F)
        fitubxii<-rubxiiarma.fit(y, ar=ar1, ma=ma1,tau=tau,link = link, X=X, X_hat=X_hat)
        tot<-tot+1
